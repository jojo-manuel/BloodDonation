...
// Try to load from file first (more reliable for development)
...
// Fallback to environment variables if file not available (for production)
if (!serviceAccount && 
  ...
  try {
    ...
    console.error('❌ Failed to parse Firebase environment variables:', error.message);
    serviceAccount = null;
  }
}

// If still no service account, Firebase will be disabled
if (!serviceAccount) {
  console.warn('⚠️  Firebase service account not found. Firebase features will be disabled.');
  console.warn('   Place firebase-service-account.json in backend/config/ to enable Firebase.');
}
